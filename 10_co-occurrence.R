...
mat <- t(mat)
# abundance <- 0.01
# table(colSums(mat)/sum(mat)>=(abundance/100))
group.test <- c("COb", "HOb", "COv", "HOv", "CN", "HN")
# group.test <- c("Normal", "Overweight", "Obesity")

spe.selected <- read.table("selected_species.txt")
spe.selected <- spe.selected[,1]
...
point.del <- 1
withname <- FALSE
pdf(paste("cooccur/", "r_", rvalue, "p_", pvalue, "del_", point.del, "_", withname,
          "_co-occurrence0616.pdf", sep = ""),
    width = 20, height = 30)
par(mfrow=c(3, 2)) #, mar=c(1,1,1,1)

lapply(1:6, function(i){
  # OTU <- mat[,colSums(mat)/sum(mat)>=(abundance/100)]
  # OTU <- mat[,spe.selected]
  OTU <- mat
  ...
  # write.table(occor.r, paste("cooccur/", group.test[i],
  #         "_cooccurrence_matrix.txt", sep = ""))
  # write.table(occor.p, paste("cooccur/", group.test[i],
  #                            "_cooccurrence_matrix_pvalue.txt", sep = ""))
  
  occor.r <- as.matrix(read.table(paste("cooccur/", group.test[i], 
                              "_cooccurrence_matrix.txt", sep = "")))
  occor.p <- as.matrix(read.table(paste("cooccur/", group.test[i],
                              "_cooccurrence_matrix_pvalue.txt", sep = "")))
